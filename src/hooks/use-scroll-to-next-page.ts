...
export function useScrollToNextPage() {
  ...
  // Reset when route changes + scroll new page to top
  useEffect(() => {
    setAtBottom(false);
  ...
  useEffect(() => {
    ...
    const checkBottom = () => {
      ...
      const isBottom = scrolled >= total - BOTTOM_THRESHOLD;
    ...
    const onWheel = (e: WheelEvent) => {
      if (atBottomRef.current && e.deltaY > 0) {
        triggerNext();
      }
    };
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      const delta = touchStartY.current - currentY; // positive = swipe up = scroll down
